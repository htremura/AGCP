...
all: steering/main.c steering/steering.c steering/steering.h driving/main.c driving/driving.c driving/comms.h
	avr-gcc -mmcu=atmega328p steering/*.c -o steering/output/steering.elf
	...
	avr-gcc -mmcu=atmega328p driving/*.c -o driving/output/driving.elf
...
steering: steering/main.c steering/steering.c steering/comms.h
	avr-gcc -mmcu=atmega328p steering/*.c -o steering/output/steering.elf
	avr-objcopy -h -S steering/output/steering.elf > steering/output/steering.lst
	avr-objcopy -j .text -j .data -O ihex steering/output/steering.elf steering/output/steering.hex
	avrdude -p m328p -c arduino -P /dev/ttyACM0 -D -U flash:w:steering/output/steering.hex:i

steering_make: steering/main.c steering/steering.c steering/comms.h
	avr-gcc -mmcu=atmega328p steering/*.c -o steering/output/steering.elf
	avr-objcopy -h -S steering/output/steering.elf > steering/output/steering.lst
	avr-objcopy -j .text -j .data -O ihex steering/output/steering.elf steering/output/steering.hex
...
driving: driving/main.c driving/driving.c driving/comms.h
	avr-gcc -mmcu=atmega328p driving/*.c -o driving/output/driving.elf
	avr-objcopy -h -S driving/output/driving.elf > driving/output/driving.lst
	avr-objcopy -j .text -j .data -O ihex driving/output/driving.elf driving/output/driving.hex
...
driving_make: driving/main.c driving/driving.c driving/comms.h
	avr-gcc -mmcu=atmega328p driving/*.c -o driving/output/driving.elf
	avr-objcopy -h -S driving/output/driving.elf > driving/output/driving.lst
	avr-objcopy -j .text -j .data -O ihex driving/output/driving.elf driving/output/driving.hex
